...
>>>>>>> .theirs
  ...
  for (cal_int = 0; cal_int < 1250;
       cal_int++) {          // Wait 5 seconds before continuing.
  ...
  for (cal_int = 0; cal_int < 2000;
    ...
    if (cal_int % 15 == 0)
      digitalWrite(
          12,
          !digitalRead(12)); // Change the led status to indicate calibration.
  ...
  // Wait until the receiver is active and the throtle is set to the lower
  // position.
  while (receiver_input_channel_3 < 990 || receiver_input_channel_3 > 1020 ||
    ...
    // We don't want the esc's to be beeping annoyingly. So let's give them a
    // 1000us puls while waiting for the receiver inputs.
    PORTD |= B11110000;      // Set digital poort 4, 5, 6 and 7 high.
    delayMicroseconds(1000); // Wait 1000us.
    PORTD &= B00001111;      // Set digital poort 4, 5, 6 and 7 low.
    delay(3);                // Wait 3 milliseconds before the next loop.
...
void loop() {
  ...
  gyro_yaw_input = (gyro_yaw_input * 0.7) +
                   ((gyro_yaw / 65.5) * 0.3); // Gyro pid input is deg/sec.
  Serial.println(F("LOOP_BEGIN"));
  ...
  angle_pitch +=
      gyro_pitch * 0.0000611; // Calculate the traveled pitch angle and add this
                              // to the angle_pitch variable.
  ...
  angle_roll +=
      angle_pitch *
      sin(gyro_yaw * 0.000001066); // If the IMU has yawed transfer the pitch
  ...
  if (start == 1 && receiver_input_channel_3 < 1050 &&
    ...
    angle_pitch =
        angle_pitch_acc; // Set the gyro pitch angle equal to the accelerometer
  ...
  pid_roll_setpoint -=
      roll_level_adjust; // Subtract the angle correction from the standardized
  ...
  pid_pitch_setpoint -=
      pitch_level_adjust; // Subtract the angle correction from the standardized
  ...
  // If the loop time is longer or shorter than 4000us the angle calculation is
  // off. If you modify the code make sure that the loop time is still 4000us and
  // no longer! More information can be found on the Q&A page:
  //! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !
  //! ! ! ! ! ! ! ! ! ! ! ! !
  ...
  // There is always 1000us of spare time. So let's do something usefull that is
  // very time consuming. Get the current gyro and receiver data and scale it to
  // degrees per second for the pid calculations.
  gyro_signalen();
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// This routine is called every time input 8, 9, 10 or 11 changed state. This is
// used to read the receiver signals. More information about this subroutine can
// be found in this video: https://youtu.be/bENjl1KQbvo
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
ISR(PCINT0_vect) {
  current_time = micros();
  // Channel 1=========================================
